...
//Journey feature done, now Container Status
	...
	public void a_registered_Container_Status_at(double hum, double temp, double press, int date) {
		aa.updateStatus(lc.getContainers().containerOnJourney(j.getID()).get(0).getID(), (float) hum, (float) temp,
				(float) press, (long) date);

	}

	@Given("a registered Container Status {double} {double} {double} at {int} for a non existing Container")
	public void a_registered_Container_Status_at_for_a_non_existing_Container(double hum, double temp, double press,
			int date) {
	...
	public void searching_for_Container_Journey_Status_History() {
		try {
			...
			cs = aa.getStatusesFromContainerOnJourney(lc.getContainers().containerOnJourney(j.getID()).get(0).getID(),
					j.getID());
		} catch (Exception e) {
			cs = new ArrayList<ContainerStatus>();
		}
